...
def _try_render(viz: GraphVisualization) -> Optional[str]:
    ...
    except Exception:  # pragma: no cover - matplotlib is installed in this build
    ...
    try:
        nodes = (
            viz.cycle_path[:-1]
            if (len(viz.cycle_path) > 1 and viz.cycle_path[0] == viz.cycle_path[-1])
            else viz.cycle_path
        )
        n = max(1, len(nodes))
        positions = {
        ...
        for e in viz.edges:
            ...
            if src in positions and dst in positions:
                ...
                ax.text(
                    (x1 + x2) / 2, (y1 + y2) / 2, f"{e['amount']:,.0f}", fontsize=7, color="darkred"
                )
        ...
        plt.close(fig)
        key = storage.put(
            buf.getvalue(), key=f"graphs/cycle_{id(viz)}.png", content_type="image/png"
        )
