...
def export_excel(report: PortfolioReport) -> str:
    ...
    key = storage.put(
        ...
        key=f"reports/portfolio_{report.period}.csv",
...
def export_pdf(report: PortfolioReport) -> str:
    ...
    lines = [
        f"PORTFOLIO QUALITY REPORT - {report.period}",
    ...
    if report.exposure_limit_breaches:
        ...
        lines += [
            f"  {b['borrower_id']}: {b['utilization_pct']}% of limit"
    ...
    try:
        ...
        for line in text.splitlines():
            if y < 40:  # pragma: no cover - page overflow only on very long reports
                pdf.showPage()
                y = height - 40
        ...
        key = storage.put(
            buf.getvalue(),
            key=f"reports/portfolio_{report.period}.pdf",
            content_type="application/pdf",
        )
    except Exception:  # pragma: no cover - reportlab is installed in this build
        key = storage.put(
            text.encode("utf-8"),
            key=f"reports/portfolio_{report.period}.txt",
