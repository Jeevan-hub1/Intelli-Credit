...
def _dim(dimension: FiveCDimension, score: float, explain: list[ExplainabilityScore]) -> DimensionScore:
...
def score_character(ctx: ScoringContext) -> DimensionScore:
    ...
    if comp and not comp.is_compliant:
        ...
        mca_score = 90.0
    explain.append(ExplainabilityScore(
        component="MCA21 Compliance",
        value=mca_score,
        reasoning=("No compliance violations found." if (comp and comp.is_compliant) or not comp
    ...
    explain.append(ExplainabilityScore(
        ...
        value=lit_score,
        reasoning=(f"{len(lit.cases)} case(s); {lit.high_risk_count} high-risk; {lit.ibc_count} IBC."
                   if lit else "No litigation found."),
    ...
    if ctx.fraud and ctx.fraud.findings:
        sev_weight = {Severity.CRITICAL: 40, Severity.HIGH: 25, Severity.MEDIUM: 12, Severity.LOW: 5}
    ...
    explain.append(ExplainabilityScore(
        ...
        value=fraud_score,
        reasoning=(f"{len(ctx.fraud.findings)} fraud finding(s); penalty {fraud_penalty:.0f} pts."
                   if ctx.fraud and ctx.fraud.findings else "No fraud signals detected."),
...
def score_capacity(ctx: ScoringContext) -> DimensionScore:
    ...
    explain.append(ExplainabilityScore(
        component="DSCR", value=dscr_score,
        reasoning=(f"DSCR {d:.2f} is below minimum {DSCR_MIN}; inadequate debt servicing."
                   if flag_dscr else f"DSCR {d:.2f} meets the {DSCR_MIN} threshold."),
        data_points={"dscr": d, "threshold": DSCR_MIN},
        source_citations=[f"Financial Statement FY{latest.fiscal_year}"]))

    icr_score = clamp(30 + i * 15)
    explain.append(ExplainabilityScore(
        component="ICR", value=icr_score,
    ...
    growth = safe_div(revs[0] - (revs[-1] if len(revs) > 1 else revs[0]),
                      revs[-1] if len(revs) > 1 else 1.0) * 100
    trend_score = clamp(55 + growth)
    explain.append(ExplainabilityScore(
        component="Revenue Trend (3yr, decay-weighted)", value=trend_score,
    ...
    explain.append(ExplainabilityScore(
        component="EBITDA Margin", value=margin_score,
        reasoning=f"EBITDA margin {margin:.1%} vs industry {ctx.industry_ebitda_margin:.1%}.",
        data_points={"ebitda_margin": round(margin, 4), "benchmark": ctx.industry_ebitda_margin}))
...
def score_capital(ctx: ScoringContext) -> DimensionScore:
    ...
    explain.append(ExplainabilityScore(
        component="Debt-to-Equity", value=de_score,
        reasoning=(f"D/E {de:.2f} exceeds {DE_PENALTY_THRESHOLD}; penalty {de_penalty:.0f} pts."
                   if de_penalty else f"D/E {de:.2f} is within acceptable range."),
    ...
    explain.append(ExplainabilityScore(
        component="Liquidity (Current/Quick)", value=liq_score,
    ...
    explain.append(ExplainabilityScore(
        component="Net Worth Trend", value=nw_score,
        reasoning=("Net worth declined for 2 consecutive years; deteriorating capital."
                   if declining_2y else "Net worth stable/improving."),
...
def score_collateral(ctx: ScoringContext) -> DimensionScore:
    ...
    if not loan or not loan.collateral:
        explain.append(ExplainabilityScore(
            component="Collateral", value=30.0,
    ...
    for item in loan.collateral:
        ...
        secured_value += net
        breakdown.append({
            "type": item.collateral_type, "market_value": item.market_value,
            "haircut": haircut, "net_value": round(net, 2),
        })
    ...
    if over_ltv:
        score = min(score, 45)
    explain.append(ExplainabilityScore(
        component="Loan-to-Value (post-haircut)", value=score,
        reasoning=(f"LTV {ltv:.0%} exceeds {LTV_MAX:.0%}; insufficient collateral coverage."
                   if over_ltv else f"LTV {ltv:.0%} within {LTV_MAX:.0%} limit."),
        data_points={"ltv": round(ltv, 4), "secured_value": round(secured_value, 2),
                     "breakdown": breakdown}))
...
def score_conditions(ctx: ScoringContext) -> DimensionScore:
    ...
    explain.append(ExplainabilityScore(
        component="Industry Growth", value=growth_score,
    ...
    explain.append(ExplainabilityScore(
        component="Regulatory Environment", value=reg_score,
    ...
    explain.append(ExplainabilityScore(
        component="News Sentiment", value=news_score,
        reasoning=(f"Adverse news {adverse_ratio:.0%} exceeds {NEGATIVE_NEWS_THRESHOLD:.0%}; "
                   f"penalty {news_penalty:.0f} pts." if news_penalty
...
def evaluate_qualitative_note(note: QualitativeNote) -> float:
    ...
    sev_factor = {Severity.CRITICAL: 1.0, Severity.HIGH: 0.75, Severity.MEDIUM: 0.5,
                  Severity.LOW: 0.25, Severity.INFO: 0.1}.get(note.severity, 0.25)
...
def apply_qualitative_notes(
    ...
    for note in notes:
        ...
        target.explainability.append(ExplainabilityScore(
...
def recommend(overall: float, critical: list[FiveCDimension], fraud: Optional[FraudReport]) -> Recommendation:
...
def synthesize(ctx: ScoringContext, notes: Optional[list[QualitativeNote]] = None) -> CreditScore:
    ...
    logger.info(
        ...
        overall, score.recommendation.value, band.value, len(critical), completeness,
    )
    return score
