...
def test_fake_itc_invalid_supplier():
    gst = GSTData(
        gstin="27A",
        supplier_gstins=["BADONE"],
        returns=[
            GSTReturn(
                form_type="GSTR-2A",
                period="2024-01",
                gstin="27A",
                entries=[GSTLineEntry(gstin="BADONE", taxable_value=600000, invoice_count=1)],
            )
        ],
    )
...
def test_mca21_and_ecourts():
    ...
    lit = search_ecourts(
        "Co",
        cases=[
            {"case_id": "1", "category": "criminal", "title": "fraud case", "monetary_value": 100}
        ],
    )
