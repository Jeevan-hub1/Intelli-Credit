...
from services.document_parser import classify, ingest_document
...
def test_document_classification_confidence(sample_financials):
    doc = ingest_document("fy23.csv", b"dummy",
                          extracted_text="balance sheet profit and loss cash flow schedule iii")
...
def test_balance_sheet_imbalance_flagged():
    fa = parse_financials([{"fiscal_year": 2023,
...
def test_gst_itc_reconciliation_flag():
    gd = parse_gst([
        {"form_type": "GSTR-2A", "period": "2024-01", "gstin": "27A", "total_itc": 100,
         "entries": [{"gstin": "27SUP1", "taxable_value": 500}]},
        {"form_type": "GSTR-3B", "period": "2024-01", "gstin": "27A", "total_itc": 120,
...
def test_bank_categorize_and_conduct():
    ...
    bs = parse_bank_statement({"account_number": "1", "transactions": [
        {"date": "2024-01-05", "description": "Salary", "credit": 50000, "balance": 150000},
        {"date": "2024-01-15", "description": "ECS Return insufficient", "debit": 0, "balance": -5000},
    ]})
