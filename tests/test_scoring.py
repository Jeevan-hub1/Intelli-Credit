...
from models.base import FIVE_C_WEIGHTS, FiveCDimension, Sentiment, Severity
...
def _ctx(sample_financials, **kw):
    ...
    loan = LoanRequest(
        amount=3000,
        tenure_months=36,
        collateral=[CollateralItem(description="P", collateral_type="property", market_value=6000)],
    )
...
def test_qualitative_note_impact_bounds():
    adverse = evaluate_qualitative_note(
        QualitativeNote(
            officer_id="o", text="t", sentiment=Sentiment.ADVERSE, severity=Severity.CRITICAL
        )
    )
    positive = evaluate_qualitative_note(
        QualitativeNote(
            officer_id="o", text="t", sentiment=Sentiment.POSITIVE, severity=Severity.CRITICAL
        )
    )
    assert adverse == -15.0
    assert positive == 8.0
